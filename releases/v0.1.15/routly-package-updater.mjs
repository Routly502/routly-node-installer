...
import { existsSync, lstatSync, readFileSync, mkdirSync, renameSync, unlinkSync, writeFileSync, openSync, fsyncSync, closeSync, readdirSync, readlinkSync, copyFileSync, cpSync, chmodSync, rmSync } from "node:fs";
...
function discardUnactivatedRelease(targetVersion) {
  if (!semver.test(targetVersion)) throw Error("invalid unactivated release version");
  const target = join(RELEASES, targetVersion);
  if (!existsSync(target)) return;
  if (existsSync(CURRENT) && lstatSync(CURRENT).isSymbolicLink() &&
      readlinkSync(CURRENT) === `/opt/routly/releases/${targetVersion}`) {
    throw Error("refusing to discard the active release");
  }
  const stat = lstatSync(target);
  if (!stat.isDirectory() || stat.isSymbolicLink() || !ownedByRoot(stat)) throw Error("unsafe unactivated release");
  const manifest = JSON.parse(readFileSync(join(target, "release-manifest.json"), "utf8"));
  if (manifest.product !== "routly-node" || manifest.version !== targetVersion) throw Error("unactivated release identity mismatch");
  const tombstone = join(RELEASES, `.discarded-${targetVersion}-${process.pid}`);
  renameSync(target, tombstone);
  syncDirectory(RELEASES);
  if (TEST_MODE && process.env.ROUTLY_TEST_EXIT_AFTER_RELEASE_TOMBSTONE === "1") process.exit(87);
...
const journal = phase => {
  ...
  const fd = openSync(temporary, "wx", 0o600); writeFileSync(fd, JSON.stringify({ phase, version, previousVersion, backup: backupPath, hostBackup: hostBackupPath, archiveSnapshot, port, at: new Date().toISOString() }) + "\n"); fsyncSync(fd); closeSync(fd); renameSync(temporary, JOURNAL); syncDirectory(UPDATER);
...
const readJournal = () => {
  ...
  if (!semver.test(value.version) || !semver.test(value.previousVersion) ||
      !["validated", "downloaded", "stopping", "stopped", "backed_up", "migrating", "migrated", "release_installed", "host_integrating", "activated", "healthy", "rolled_back", "manual_intervention"].includes(value.phase)) {
...
function normalizeReleaseTree(path) {
  const stat = lstatSync(path);
  if (stat.isSymbolicLink()) return;
  if (stat.isDirectory()) {
    chmodSync(path, 0o755);
    for (const entry of readdirSync(path)) normalizeReleaseTree(join(path, entry));
    return;
  }
  if (stat.isFile()) {
  ...
  throw Error(`unsupported release entry type: ${path}`);
}
...
try {
  ...
  if (interrupted?.phase === "healthy") {
    port = interrupted.port || 4100;
    hostBackupPath = interrupted.hostBackup || "";
    archiveSnapshot = interrupted.archiveSnapshot || "";
    let activeStat;
    try { activeStat = lstatSync(CURRENT); } catch (error) { if (error.code !== "ENOENT") throw error; }
    if (!activeStat?.isSymbolicLink() || readlinkSync(CURRENT) !== `/opt/routly/releases/${interrupted.version}`) {
      journal("manual_intervention");
      throw Error("MANUAL INTERVENTION REQUIRED: healthy update is not the active release");
    }
    run(CURL, probeArgs("/api/healthz"));
    if (readProbe("/api/version") !== interrupted.version) {
      journal("manual_intervention");
      throw Error("MANUAL INTERVENTION REQUIRED: healthy update reports a different version");
    }
    if (hostBackupPath) rmSync(hostBackupPath, { recursive: true, force: true });
    if (archiveSnapshot) rmSync(dirname(archiveSnapshot), { recursive: true, force: true });
    removeJournal();
    process.exit(0);
  }
  if (interrupted && interrupted.phase !== "healthy" && interrupted.phase !== "rolled_back") {
    ...
    archiveSnapshot = interrupted.archiveSnapshot || "";
    port = interrupted.port || 4100;
    if (["stopping", "stopped", "backed_up"].includes(interrupted.phase)) {
    ...
    } else if (["migrating", "migrated", "release_installed", "host_integrating", "activated"].includes(interrupted.phase)) {
      try {
        ...
        discardUnactivatedRelease(interrupted.version);
  ...
  const installed = join(RELEASES, version); mkdirSync(RELEASES, { recursive: true, mode: 0o755 }); if (existsSync(installed)) throw Error("release already installed");
   // The hardened systemd unit exposes /var/lib and /opt as separate bind
   // mounts. A direct rename from the staging tree can therefore fail with
   // EXDEV even when both paths share the same underlying disk. Copy into the
   // destination mount first, sync it, then rename atomically within /opt.
   const incoming = join(RELEASES, `.incoming-${version}`);
   rmSync(incoming, { recursive: true, force: true });
   cpSync(candidate, incoming, { recursive: true, force: false, errorOnExist: true, preserveTimestamps: true });
    // Extraction runs under umask 077, so normalize the already verified
    // immutable tree before activation. Package files contain no secrets.
    normalizeReleaseTree(incoming);
   syncTree(incoming);
  ...
  console.error(error.message);
   if (apiStopped && !backupVerified) {
    ...
    if (backupPath && existsSync(backupPath)) {
      ...
      const prior = readProbe("/api/version");
      if (prior !== previousVersion) throw Error(`rollback reported ${prior}, expected ${previousVersion}`);
      discardUnactivatedRelease(version);
      journal("rolled_back");
    }
